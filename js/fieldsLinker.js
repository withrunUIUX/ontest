...
function FieldsLinker(selector){
    ...
    this.move = null;
    this.that = null;
    this.lineStyle = 'square-ends'; // straight or square-ends
    this.handleColor = '#0098ED'.split(',');
    ...
    this.draw = function () {
        ...
        self.canvasCtx.lineWidth = 2; // 선두께
   ...
   this.makeLink = function (infos) {
    ...
    this.fillChosenLists = function () {
        ...
        self.listAtype = self.data.Lists[0].type || '';
        self.listBtype = self.data.Lists[1].type || '';
    ...
    this.drawColumnsAtLoadTime = function () {
        ...
        self.$ulLeft
            ...
            //         .attr('type', 'button')
            //         .text('선 지우기')
            //         .addClass('button is-info eraseLink')
            //         .attr("style","position:absolute;top:-6px;opacity:0.9;width:75px;height:35px;font-size:15px;margin-left: 5.5%;")
    ...
    this.drawColumnsContentA = function () {
        ...
        self.listA.forEach(function (x, i) {
            ...
            if(self.listAtype[i] == 'img'){
                ...
                $img
                    .appendTo($div)
                    .addClass(id)
                    .attr('src', self.data.filepath.fileListA[i])
                    .css({
                       /* 'width': '75%',
                        'height': '100%',
                        '-webkit-user-select':'none',
                    ...
                    var $span = $('<span class="FL-tt"></span>');
                    // if(self.data.filepath.fileListA[i].length > 36){
                    $span
                        .appendTo($div)
                        .css({
                         /*   'width': '95%',
                            ...
                            .html(self.data.filepath.fileListA[i]);
                    ...
                    //     .html(self.data.filepath.fileListA[i]);
                    // }
            }else if(self.listAtype[i] == 'sound'){
                var $video = $('<audio controls></audio>');
                $video
                    .appendTo($li)
                    .addClass(id)
                    .attr('ondrop', 'LM_drop(event)')
                ...
                $source
                    ...
                    .attr('type', "video/mp4")
                    .html(item);  
            }else if(self.listAtype[i] == 'movie'){
        ...
        if (self.isTouchScreen && !self.mobileClickIt) {
            ...
            $(self.$ulLeft).find('li').off('touchstart').on('touchstart', function (e) {
                e.preventDefault();
                if (self.isDisabled) return;
                self.move = {};
            ...
            $(self.$ulLeft).find('li').off('touchmove').on('touchmove', function (e) {
                e.preventDefault();
                var originalEvent = e.originalEvent;
                if (originalEvent != null && originalEvent.touches != undefined) {
                    var touch = originalEvent.touches[0];
            ...
            $(self.$ulLeft).find('li').off('touchend').on('touchend', function (e) {
                if (self.isDisabled) return;
                var originalEvent = e.originalEvent;
                if (originalEvent != null && originalEvent.touches != undefined) {
        ...
        // 터치 후 다시 라인 지울때 작업 필요함
        // $(self.$root).find('.unlink').off('touchstart').on('touchstart', function (e) {
        //     if (self.isDisabled) return;
        //     self.eraseLinkA($(this).data('name'));
        //     self.draw();
        // });
   ...
   this.drawColumnsContentB = function () {
        ...
        self.listB.forEach(function (x, i) {
            ...
            if(self.listBtype[i] == 'img'){
                    ...
                    $img
                        .appendTo($div)
                        .addClass(id)
                        .attr('src', self.data.filepath.fileListB[i])
                        .css({
                           /* 'width': '75%',
                            'height': '100%',
                            '-webkit-user-select':'none',
                    ...
                    $span
                        ...
                        .html(self.data.filepath.fileListB[i]);

            }else if(self.listBtype[i] == 'sound'){
                var $video = $('<audio controls></audio>');
                $video
                    .appendTo($li)
                    .addClass(id)
                ...
                $source
                    ...
                    .attr('type', "video/mp4")
                    .html(item);  
            }else if(self.listBtype[i] == 'movie'){
                var $video = $('<video controls></video>');
                $video
                    .appendTo($li)
                    .addClass(id)
    ...
    this.drawImmediate = function (e) {
        // 여기가 드래그 했을 때 선이 따라오는 곳
        // draw가 선과 선이 맞닿을 때 처리되는 곳
        var self = this;
        self.canvasCtx.clearRect(0, 0, self.canvasWidth, self.canvasHeight);
        ...
        self.canvasCtx.strokeStyle = color;
        var Ax = 0;
        var Ay = self.ListHeights1[_from];   // 그에 따라 해당 높이값도 못받아오기 때문에 선이 안그어짐
        // var Ay = 98;

        // mouse position relative to the canvas
        //var Bx = e.offsetX;
        //var By = e.offsetY;
        var relativePosition = self.getTouchPos(e); // 캔버스 위에 드래그가 되고있는 좌표값 
